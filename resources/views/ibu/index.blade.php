<x-layout>
  ...
  <section class="m-1 p-8 rounded-md bg-white">
    ...
    <div class="flex justify-between items-center my-4 mx-8">
      <a href="/ibu/create" class="m-2 px-8 py-2 font-semibold text-white rounded-md bg-green-500 hover:bg-green-600">
        Tambah Data
      </a>
    ...
    <table class="m-2 table-auto min-w-full border-collapse border border-slate-400 rounded-lg overflow-hidden shadow-lg items-center text-center">
      ...
      <tbody id="tBody-ibu">
        ...
        <tr>
          ...
          <td class="border px-4 py-2 border-slate-300">{{ \Carbon\Carbon::parse($ibu->tgl_lahir)->age }}</td>
          <td class="border px-4 py-2 border-slate-300">{{ $ibu->is_hamil }}</td>
          <td class="border px-4 py-2 border-slate-300">{{ $ibu->alamat }}</td>
          <td class="border px-4 py-2 border-slate-300">
            <a href="{{ route('ibu.show', $ibu->id) }}" class="btn btn-info">Lihat</a>
            <a href="{{ route('ibu.edit', $ibu->id) }}" class="btn btn-info">Ubah</a>
